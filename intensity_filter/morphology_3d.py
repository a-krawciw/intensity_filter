...
def filter_by_radius(points: np.ndarray, max_dist=0.1):
    """Remove all points that are further than max_dist (m) from set
    points: nx3"""

    if points.shape()[1] != 3:
        raise RuntimeError("Requires 3D points")
    tree = KDTree(points, metric='euclidean')
    idx_counts = tree.query_radius(points, r=max_dist, count_only=True)
    return points[idx_counts > 1]
